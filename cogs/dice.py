...
class Dice(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.json_file_path = 'settings/dice.json'
    ...
    @commands.slash_command(name="dice", description="Roll a random multiplier between 10 and 99.99")
    async def dice(self, inter: disnake.ApplicationCommandInteraction):
        ...
        multiplier, _ = self.generate_random_result()
        ...
        embed = disnake.Embed(
            ...
            description=settings['embed_settings']['description'].format(multiplier=multiplier),
        ...
        async def reroll_callback(interaction: disnake.MessageInteraction):
            ...
            embed.description = settings['embed_settings']['description'].format(multiplier=multiplier)
    ...
    def generate_random_result(self):
        # Generate a random number between 10 and 99.99
        multiplier = round(random.uniform(10, 99.99), 2)
        odds = 0.0  # Not used, but keeping the structure intact

        return multiplier, odds

def setup(bot):
    bot.add_cog(Dice(bot))
print(f'{Colors.WHITE} +{Colors.GREEN} dice.py{Colors.RESET}')
